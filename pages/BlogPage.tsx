...
const BlogPage: React.FC = () => {
  ...
  const filteredPosts = blogPosts.filter(post => {
    ...
    // Search in title, excerpt, and all content blocks
    const matchesSearch = !query || 
      post.title.toLowerCase().includes(query) ||
      post.excerpt.toLowerCase().includes(query) ||
      post.content.some(block => block.text && block.text.toLowerCase().includes(query));
  ...
  // Hide featured section if user is searching to focus on results
    ...
    <>
      ...
      <div className="bg-brand-primary min-h-screen overflow-x-hidden">
        ...
        <div className="container mx-auto px-4 md:px-6 py-12">
          ...
          {/* Featured Article - Hidden when searching */}
          {showFeatured && (
          ...
          {filteredPosts.length === 0 && (
               <div className="text-center py-20 text-brand-muted">
                  ...
                  <button 
                      ...
                      className="text-brand-accent-middle hover:text-white font-semibold transition-colors underline underline-offset-4"
                  >
                      Clear all filters
                  </button>
               </div>
